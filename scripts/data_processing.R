...
# Read the dataset
data <- readRDS('dataset/final_all_ctc_blood_raw_count.rds')
...
# Apply cell filter
cell_filter_data <- cell_filter(data,6)
...
CTC_GSE111065_sample = grep("Ctc_GSE111065",colnames(gene_filter_data))
...
# Subsampling the Blood_Genomics_sample
Blood_Genomics_sample=Blood_Genomics_sample[colSums(gene_filter_data[,Blood_Genomics_sample])>5000]
...
# Total samples after sub-sampling
length(index)
...
saveRDS(normalization_data,file='dataset/final_all_ctc_blood_sub_sampled_normalized_data.rds')
...
dim(ml_data)
saveRDS(ml_data,file='dataset/ml_median_normalized_data.rds')
