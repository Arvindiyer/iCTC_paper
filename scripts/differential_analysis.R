...
# Load the dataset.
normalized_data <- readRDS('dataset/final_all_ctc_blood_sub_sampled_normalized_data.rds')
dim(normalized_data)
length(grep("Ctc_", colnames(normalized_data)))

# Perform Wilcoxon test on ctc vs blood and compute pvalues
pval<-apply(normalized_data,1,function(x) wilcox.test(x[1:737],x[738:3079])$p.value)
...
FC = log2(Matrix::rowMeans(normalized_data[,1:737])/Matrix::rowMeans(normalized_data[,738:3079]))
...
dim(reduced_df)
genes<-read.csv('dataset/cell_surface_marker.csv',sep = "\t",header = TRUE)
...
express <- apply(new_Data[,1:737],1,function(x) sum(x>0)>=(515))
sum(express)
...
expression_data<-normalized_data[rownames(data_df),1:737]
rowMeans(expression_data)
View(rowMeans(expression_data))
# Generate Average Expression for Blood
expression_data<-normalized_data[rownames(data_df),738:3079]
rowMeans(expression_data)
View(rowMeans(expression_data))